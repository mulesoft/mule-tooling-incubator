...
public class GetIconVisitor implements IElementVisitor {
    ...
    public void visit(CloudConnectorMessageSource cloudConnectorMessageSource) {
        label = String.format("CloudConnectorMessageSource (%1)", cloudConnectorMessageSource.getLocalId());

    }

    @Override
    public void visit(Global global) {
        label = String.format("Global (%1) %2", global.getLocalId(),global.getCaption());
    }
    ...
    public void visit(AttributeCategory attributeCategory) {
        label = doGetLabel(attributeCategory);
