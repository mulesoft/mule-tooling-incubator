...
public class AddSampleQuickFix extends QuickFix {
    ...
    protected ASTRewrite getFix(CompilationUnit unit, Integer errorMarkerStart) {
        ...
        if (visitor.getNode() != null) {
            goToSampleInDocSampleFile(compilationUnit, visitor.getNode(), modelVisitor.getRoot().getModules().get(0));
    ...
    private void goToSampleInDocSampleFile(ICompilationUnit unit, ASTNode node, Module module) {
        IFile file = null;
        IResource resourceFile = null;
        IFolder folder = unit.getJavaProject().getProject().getFolder(DevkitUtils.DOCS_FOLDER);
        ...
        for (ModuleMethod moduleMethod : module.getProcessor()) {
            if (moduleMethod.getMethod().getName().toString().equals(methodName)) {
                method = moduleMethod;
            }
        }
        try {
            for (IResource resource : folder.members()) {
                if (resource.getName().matches(".*.sample")) {
                    file = unit.getJavaProject().getProject().getFile(resource.getProjectRelativePath());
        ...
        try {
            ...
            Writer writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(file.getRawLocation().toFile(), true), "UTF-8"));

            writer.append(String.format("\n<!-- BEGIN_INCLUDE(%2$s) -->\n" + "\t<%1$s:%3$s/>\n" + "<!-- END_INCLUDE(%2$s) -->\n", ModuleUtils.getTargetNameSpace(module), sample,
                    processorName));
            writer.close();
            folder.refreshLocal(IResource.DEPTH_ONE, null);
            file = unit.getJavaProject().getProject().getFile(resourceFile.getProjectRelativePath());
