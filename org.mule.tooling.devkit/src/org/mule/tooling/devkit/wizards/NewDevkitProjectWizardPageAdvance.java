...
public class NewDevkitProjectWizardPageAdvance extends WizardPage implements IModelPopulator<ProjectBuilder> {
    ...
    private void updateComponentsEnablement() {
        ...
        groupId.setEnabled(useDefaults);
        artifactId.setEnabled(useDefaults);
        version.setEnabled(useDefaults);
    }
    ...
    private void addMavenGroup(Composite container) {
        ...
        useDefaultValuesCheckbox = new Button(mavenGroupBox, SWT.CHECK);
        useDefaultValuesCheckbox.setSelection(true);
        useDefaultValuesCheckbox.setText(" " + CREATE_POM_LABEL);
        useDefaultValuesCheckbox.setLayoutData(GridDataFactory.swtDefaults().span(2, 1).create());
