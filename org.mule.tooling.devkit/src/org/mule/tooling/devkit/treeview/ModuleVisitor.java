...
public class ModuleVisitor extends ASTVisitor {
	...
	public boolean visit(NormalAnnotation node) {
		if (node.getParent() instanceof MethodDeclaration) {
			if (ModelUtils.isAnnotationSupported(node.getTypeName().toString())) {
				...
				method.setConnectionMethod(ModelUtils
						.isConnectionAnnotation(node.getTypeName().toString()));
				...
				for (Object value : node.values()) {
					...
					if (valuePair.getValue() instanceof StringLiteral) {
						prop.setValue(((StringLiteral) valuePair.getValue())
								.getLiteralValue());
					} else {
		...
		module.setName(((TypeDeclaration) node.getParent()).getName()
				.toString());
	...
	public boolean visit(MarkerAnnotation node) {
		if (node.getTypeName().toString().equals("Configurable")) {
			...
			method.setMetadataMethod(ModelUtils.isMetadaMethod(node
