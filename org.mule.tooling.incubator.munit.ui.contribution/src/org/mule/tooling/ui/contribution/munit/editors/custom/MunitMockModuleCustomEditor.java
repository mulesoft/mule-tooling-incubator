...
public abstract class MunitMockModuleCustomEditor extends CustomEditor implements IForceSave {
    ...
    /**
     * Draw the particular form for the sub class extending this class. For example, the Spy message processor should draw the spy action form in this method, the mock message
     * processor should draw the "then-return" form here.
     */
    protected abstract void drawCustomFormIn(Composite parentEditorGroup);

    /**
     * Each custom editor must search for its parent because of the studio framework, this method forces the sub classes of the {@link MunitMockModuleCustomEditor} to define the
     * parent identifier. It has to be the same as specified in the mock.xml file.
     */
    ...
    public MunitMockModuleCustomEditor(AttributesPropertyPage parentPage, AttributeHelper helper) {
    ...
    @Override
    public boolean hasToForceSave(MuleConfiguration currentMuleConfiguration, IMuleProject muleProject, MessageFlowEntity messageFlowEntity) {
        return true;
    }
