...
/**
 * Model helper class to help computing auto-completion suggestions. This class
 * is in charge of parsing the script and extracting all the information that
 * would be necessary. This is as well a best-effort approach since we don't
 * have the ability to really introspect objects.
 * 
...
public class GradleScriptAutocompleteAnalyzer {
	...
	public GradleScriptAutocompleteAnalyzer(IDocument gradleScript,
			String completionWord, int position,
			GradleMuleBuildModelProvider modelProvider) {
	...
	private List<GroovyCompletionSuggestion> buildCompletionSuggestions()
			throws Exception {

		// get the current context of auto completion.
		Class<?> currentContext = buildContextClass();
		DSLMethodAndMap dslMethod = ScriptParsingUtils.parseDSLLine(
				gradleScript, insertPosition);

		boolean lineIsPropertyAccess = !StringUtils.isEmpty(parseLeftSide());

		if (lineIsDslMethodCall(dslMethod, currentContext)
				&& !lineIsPropertyAccess) {
			DSLCompletionStrategy strategy = buildDslCompletionStrategy(
					currentContext, dslMethod);
			String expectedInputKey = currentAssignmentVariableName();

			if (strategy == null) {
				return Collections.emptyList();
			} else {
				return strategy.buildSuggestions(dslMethod, currentContext,
						expectedInputKey);
			}
		}
		...
		if (!StringUtils.isEmpty(assignmentProperty)) {
			...
			for (Object key : props.keySet()) {
				ret.add(new GroovyCompletionSuggestion(
						GroovyCompletionSuggestionType.RAW_VALUE, key
								.toString(), "Global build properties"));
			}
		}

		if (currentContext != null) {
			ret.addAll(ObjectMetadataCache
					.buildAndCacheSuggestions(currentContext));
			...
			for (GradleMulePlugin p : GradleMulePlugin.values()) {
				if (isPluginVisible(p)) {
					ret.add(new GroovyCompletionSuggestion(
							GroovyCompletionSuggestionType.PROPERTY, p
									.getExtensionVariableName(), p
	...
	private DSLCompletionStrategy buildDslCompletionStrategy(
			Class<?> currentContext, DSLMethodAndMap dslMethod) {

		if (GradleMulePlugin.STUDIO.getExtensionClass().equals(currentContext)) {
			return new MuleComponentsDSLCompletionStrategy();
		}

		else if (GradleMulePlugin.CLOUDHUB.getExtensionClass().equals(
				currentContext)) {
			return new CloudhubDSLCompletionStrategy();
		}

		else if (GradleMulePlugin.MMC.getExtensionClass()
	...
	private Class<?> buildContextClass() throws Exception {
		...
		boolean isInComponentsContext = ScriptParsingUtils
				.isPositionInClosureContext(
						gradleScript,
						MuleGradleProjectCompletionMetadata.COMPONENTS_CLOSURE_SCOPE,
						insertPosition);
		boolean isCloudhubContext = ScriptParsingUtils
				.isPositionInClosureContext(
						gradleScript,
						MuleGradleProjectCompletionMetadata.CLOUDHUB_DOMAINS_CLOSURE_SCOPE,
						insertPosition);
		boolean isMMCContext = ScriptParsingUtils
				.isPositionInClosureContext(
						gradleScript,
						MuleGradleProjectCompletionMetadata.MMC_ENVIRONMENTS_CLOSURE_SCOPE,
						insertPosition);
		boolean isDependenciesContext = ScriptParsingUtils
				.isPositionInClosureContext(
						gradleScript,
						MuleGradleProjectCompletionMetadata.DEPENDENCIES_CLOSURE_SCOPE,
						insertPosition);
	 ...
	 * @return
	 */
	private boolean isPluginVisible(GradleMulePlugin plugin)
		...
		int pluginLine = modelProvider.getAppliedMulePlugins().get(plugin)
				.getSourceNode().getLineNumber();
	...
	private boolean lineIsDslMethodCall(DSLMethodAndMap method, Class<?> context)
			throws BadLocationException {
		...
		if (DslReflectionUtils.contextContainsDSLMethod(method.getMethodName(),
				context)) {
			return true;
		}
	...
	protected String getLineOfPosition() throws BadLocationException {
		IRegion lineRegion = gradleScript
				.getLineInformationOfOffset(insertPosition);

		int length = insertPosition - completionWord.length()
		...
		// we are actually interested until the cursor position without the last
		// typed word
	...
	/**
	 * Checks the position to verify if we're in the presence of an assignment
	 * statement. If it is, it will return the name of the variable.
	 * 
	 * @return
	 */
